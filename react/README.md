...
- A TextFieldElement: It takes in the value of the textfield and a onChange function to call when the textfield changes. Note that the textfield doens't have a state to store the current value. That job is for a view. (Tips is to use GammaForm from [react-digit-components](#react-digit-components) here)
...
See [Declarative design](#declarative-design)

- **Utils**

Utils should only contain functions or constants.

_Things that can be in Utils:_
...
Should retrieve local data, e.g. localStorage.
